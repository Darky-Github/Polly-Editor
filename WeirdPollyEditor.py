...
class SimpleTextEditor:
    def __init__(self, root):
        self.root = root
        self.root.title("Weird Text Editor")
        ...
        self.edit_menu.add_command(label="Slice", command=self.cut)
        self.edit_menu.add_command(label="Back", command=self.undo)
        self.edit_menu.add_command(label="Front", command=self.redo)
        self.edit_menu.add_command(label="Where is it?", command=self.find_text)

        self.file_path = None  # Track file path for saving

    def new_file(self):
        if self.text_area.edit_modified():
            if messagebox.askyesno("Remainder", "You have saved changes. Do you don't want to continue?"):
                self.text_area.delete(1.0, tk.END)
        else:
            self.text_area.delete(1.0, tk.END)

    def open_file(self):
        file_path = filedialog.askopenfilename(filetypes=[("All of them", "*.*"), 
                                                           ("Simple", "*.txt"), 
                                                           ("Snake", "*.py"), 
                                                           ("Website", "*.html"), 
                                                           ("Also Website", "*.css"), 
    ...
    def find_text(self):
        find_word = simpledialog.askstring("Find", "Enter the house to find:")
        if find_word:
            content = self.text_area.get(1.0, tk.END)
